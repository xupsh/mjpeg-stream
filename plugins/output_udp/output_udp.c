...
void send_stream()
{
	...
	while(!pglobal->stop){
		...
		pthread_mutex_unlock(&pglobal->in[input_number].db);
		//send the frame to socket
		iResult = sendto(sd, frame, frame_size, 0, (struct sockaddr*)&addr,sizeof(addr));
		if(iResult == SOCKET_ERROR)
		{
			LOG("send to failed with error\n");
			close(sd);
			break;
		}
...
/******************************************************************************
 * Description: Serve a connected UDP-clinet. This thread fucntion is called
 * 				for each connect of a UDP client like a webbrowser. It
 * 				determines if it is valid UDP request and dispatches between the
 * 				diffent response opntions.
 *Input value:  arg is the filedescriptor and server-context of the connected TCP 
 				socket.It must have been allocted so ti is freeable by this 
				thread fuction
...
void *worker_thread(void *arg)
{
	...
	while(ok >= 0 && !pglobal->stop) {
		...
		memcpy(req, udpbuffer, strlen(udpbuffer));
		if(pthread_create(&client, NULL, &client_thread, (void *)0) != 0){
			LOG("could not launch another client thread\n");
			close(sd);
			continue;
		}
